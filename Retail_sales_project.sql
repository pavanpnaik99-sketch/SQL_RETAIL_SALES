...
SELECT 
    category,
    CONCAT(ROUND(SUM(total_sale - cogs) / 1000, 2), 'k') AS total_profit_k
FROM retail_sales
GROUP BY category
...
SELECT 
    sale_date,
    SUM(total_sale) OVER (ORDER BY sale_date) AS running_sales
FROM retail_sales;
...
SELECT 
    ...
    ROUND(SUM(total_sale) * 100.0 / 
         (SELECT SUM(total_sale) FROM retail_sales), 2) AS contribution_pct
FROM retail_sales
GROUP BY category;
